<script>
	...
	const blockTypes = [
		...
		{ type: 'newsletterEmbed', label: 'Newsletter form' }
	];
	...
	let blocks = $state((data.page.blocks ?? []).map(normalizeBlock));
	let payload = $derived(
		JSON.stringify({
			title: page.title,
			slug: page.slug,
	...
	function normalizeContent(type, content) {
		...
		if (type === 'newsletterEmbed') {
			return {
				heading: content.heading ?? '',
				bodyHtml: content.bodyHtml ?? '',
				uid: content.uid ?? '132ea7cb7b'
			};
		}
		return {
			heading: content.heading ?? '',
	...
	async function uploadImage(event) {
		...
		try {
			...
			const response = await fetch('/pages/images/upload', {
				method: 'POST',
				body: formData
			});
	...
	function blockSummary(block) {
		...
		if (block.type === 'newsletterEmbed') return block.content.heading || block.content.uid || 'Newsletter form';
		return block.type;
	}
...
<svelte:head>
	<title>{page.title || 'Page'} | C.L. Blossom Admin</title>
</svelte:head>

<section class="space-y-6">
	<div class="flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-3xl font-black text-gray-950">{page.title || 'Page'}</h1>
			<p class="mt-1 text-sm font-semibold text-gray-600">Build this page from ordered content blocks.</p>
		</div>
	...
	{/if}
	{#if form?.saved}
		<p class="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm font-semibold text-green-800">Page saved.</p>
	{/if}
	{#if uploadError}
	...
	<form method="POST" action="?/save" class="space-y-4">
		...
		<section class="panel grid gap-4 p-5 md:grid-cols-2">
			<div class="field">
				...
				<input class="input" bind:value={page.title} required />
			</div>
			<div class="field">
				<span class="label">Page address</span>
				<input class="input" bind:value={page.slug} disabled={data.page.isRequired || data.page.isProtected} required />
				<p class="text-xs font-semibold text-gray-500">
					{data.page.path ?? '/'}{data.page.isRequired || data.page.isProtected ? ' is locked for this built-in page.' : ''}
				</p>
			</div>
			<div class="field md:col-span-2">
				<span class="label">Search description</span>
				<textarea class="textarea" rows="3" bind:value={page.metaDescription}></textarea>
			</div>
			<label class="flex items-center gap-2 text-sm font-black text-gray-700">
				<input type="checkbox" bind:checked={page.isPublished} disabled={data.page.isRequired} />
				Published
			</label>
		</section>
		...
		>
				...
				<section animate:flip={{ duration: flipDurationMs }} class="panel overflow-hidden">
					...
					{#if !isCollapsed(block)}
						<div id="block-editor-{block.id}" class="grid gap-4 p-5" transition:slide={{ duration: 160 }}>
							...
							{:else if block.type === 'newsletterEmbed'}
								<div class="field">
								<span class="label">Heading</span>
								<input class="input" bind:value={block.content.heading} />
								</div>
								...
								<RichTextEditor id="newsletter-body-{block.id}" bind:value={block.content.bodyHtml} />
								</div>
								<div class="field">
								<span class="label">Kit form id</span>
								<input class="input" bind:value={block.content.uid} required />
								</div>
							{/if}
						</div>
					{/if}
			...
			{:else}
				<p class="panel p-5 text-sm font-semibold text-gray-600">No page blocks yet.</p>
			{/each}
		</div>

		<div class="flex flex-wrap justify-between gap-3">
			{#if !data.page.isRequired && !data.page.isProtected}
				<button class="button button-secondary" type="submit" formaction="?/delete">Delete page</button>
			{/if}
